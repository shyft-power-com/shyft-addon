...
* **Neu: Hilfe-Assistent kann Log und Problembeschreibung an das Shyft-Team senden.** Geht es in einer Antwort um ein Integrationsproblem oder einen Fehler, den die KI nicht sicher lösen kann, hängt sie am Ende die Markierung `[[SUPPORT_ANGEBOT: kurze Problembeschreibung]]` an. Der Chat zeigt die Markierung nie an, sondern (höchstens einmal pro Chat) ein Formular: bearbeitbare Beschreibung, optionale E-Mail-Adresse für eine Rückmeldung, ein Hinweis auf die gesendeten Daten und der Button „An shyft-power senden“. Sobald das Formular erscheint, ist das ausführliche Logging für 5 Minuten aktiviert (mit Countdown und der Bitte, das Problem noch einmal nachzustellen); danach schaltet es sich von selbst auf den Wert der Add-on-Option zurück. Der Klick auf „An shyft-power senden“ stellt die Anfrage ein: nach Ablauf der 5 Minuten (auch bei geschlossenem Chat) holt das Add-on das Log der letzten 5 Minuten, schwärzt Zugangsdaten (Supervisor-Token, `shyft_access_key`, Token/Passwörter), begrenzt es auf 1 MB (die neuesten Zeilen bleiben, einzelne Zeilen werden auf 2.000 Zeichen gekürzt) und sendet es an den Bubble-Workflow `ha_addon_error_logging` (Test bzw. Produktion je nach Zugangsschlüssel): `user` = Nutzer-ID, `meta` = Beschreibung, Leerzeile, `addon_version=…` und ggf. `email=…`, `error_type` = `service_request`, `error_message` = das Log. Der Status (wartet/gesendet/Fehler) erscheint im Formular. Ohne shyft-power-Konto (Demomodus) ist das Senden nicht möglich; ein Add-on-Neustart während des Wartens bricht den Versand ab (das Formular weist darauf hin). Der bestehende Fehlerreport bleibt unverändert.
...
* **Fix: „Laden beenden“ setzte einen fehlgeschlagenen Wallbox-Test wieder auf „bestanden“.** Der Beenden-Schritt des Tests speicherte sein Ergebnis unter demselben Schlüssel wie der Start-Test, sodass ein erfolgreiches Beenden nach einem fehlgeschlagenen Start den Test serverseitig als bestanden markierte (die Anzeige zeigte weiter den Fehler). Erfolge des Beenden-Schritts werden nicht mehr gespeichert, Fehlschläge weiterhin.
...
* **Ersparnis mehrstündiger PV-Überschuss-Sessions wird je Stunde bewertet.** Die geladene Energie wird an den Stundengrenzen aufgeteilt und mit `p_buy`/`p_sell` der jeweiligen Stunde bewertet und summiert. Die Preise einer Stunde werden bei deren Beginn in der Session festgehalten (der Dashboard-Cache enthält nur den jeweils letzten Optimierungslauf, vergangene Stunden ließen sich sonst nicht mehr nachschlagen).
* **Karten-Untertitel zeigt die geladene Energie statt der Ziel-Leistung.** „PV-Überschussladen (5.4 kW)“ zeigte nur den zuletzt eingestellten Zielwert der Regelung und ließ sich nicht einordnen; jetzt steht dort die bisher (bzw. insgesamt) geladene Energie, z. B. „PV-Überschussladen (3.1 kWh)“.
...
* **Analyse-Tab: Stundenbezeichnung korrigiert** - statt "1909., 21 Uhr Uhr" steht dort jetzt "19.09., 21 Uhr".
...
* **Fix: Der Basisfall ("was ohne Shyft-Optimierung passiert wäre") startete jede stündliche Neuberechnung mit dem ECHTEN, vom Optimierer bereits günstig vorbereiteten Zustand** (vorgeheiztes Haus, voller Heimspeicher, warmes Wasser, Auto-Ladestand) - er "erbte" so faktisch den Erfolg der Optimierung, ohne je für eine eigene (schlechtere) Vorstunden-Entscheidung zu bezahlen. Sichtbar wurde das im neuen Analyse-Tab: die Basisfall-Kosten lagen über mehrere zusammenhängende Stunden exakt bei 0 €, obwohl unverändert Grundlast anfiel (der simulierte Speicher deckte sie im Basisfall aus einem Ladestand, der real nur dank der Optimierung so hoch war).
  * `base_case.compute_base_case()` führt jetzt optional seine eigene, unabhängige Zustands-Trajektorie über Stunden hinweg fort (Raumtemperatur, Warmwassertank-Temperatur, Auto- und Heimspeicher-Ladestand) statt sie bei jedem Lauf aus `input_csv` zu übernehmen - Nachfrage-/Wetter-/Preisreihen kommen weiterhin unverändert aus dem jeweils aktuellsten `input_csv`. Nur beim allerersten Lauf sowie nach einer Lücke (Neustart, übersprungener Sync) fällt der Startzustand auf `input_csv` zurück (`nextState`/`state_overrides`, persistiert in `/data/base_case_running_state.json`).
  * Das macht eine einzelne Stunde weniger direkt mit dem Optimierer vergleichbar, dafür wird der Vergleich über längere Zeiträume korrekt - wirkt sich auf alle Verbraucher des Basisfalls aus: den Dashboard-Vergleichs-Chart, die Ersparnis-pro-Aktion-Berechnung UND das Energie-Archiv (Analyse-Tab).
...
* **Robustheit: Ein einzelner fehlgeschlagener Awattar-Abruf (Netzwerkfehler) ließ den berechneten Strompreis für diesen stündlichen Sync komplett ausfallen**, obwohl noch ein (nur leicht veralteter) Cache-Stand vorlag - dieser wird jetzt als Rückfall verwendet, statt den kompletten Sync ohne Preisangabe an den Server zu schicken.
* **Fix: Für zukünftige ("geplant") Stunden wurde eine "Auto laden"-Aktion auch dann eingeplant, wenn die Anwesenheitsprognose das Auto für diese Stunde als abwesend (aber nicht fahrend, Status "steht") vorhersagte.** Ursache: dem Optimierer werden über ev_usage_h/d_ev_kwh nur die vorhergesagten Fahrstunden mitgeteilt - eine vorhergesagte "steht"-Stunde sieht für ihn wie eine ganz normale Zuhause-Stunde aus. Die Ladeplanung prüft für alle Stunden außer der laufenden jetzt zusätzlich die Anwesenheitsprognose und überspringt die Stunde, wenn "eingesteckt" nicht der wahrscheinlichste Zustand ist. Die laufende Stunde bleibt unverändert allein vom Live-Wallbox-Status abhängig.
